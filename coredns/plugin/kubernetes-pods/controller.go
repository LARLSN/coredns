...
const (
	...
	podNameNamespaceIndex = "EndpointNameNamespace"
)
...
func newdnsController(ctx context.Context, kubeClient kubernetes.Interface, opts dnsControlOpts) *dnsControl {
	...
	if opts.initPodCache {
		dns.podLister, dns.podController = object.NewIndexerInformer(
			...
			&api.Pod{},
			cache.ResourceEventHandlerFuncs{AddFunc: dns.Add, UpdateFunc: dns.Update, DeleteFunc: dns.Delete},
			cache.Indexers{podNameNamespaceIndex: podNameNamespaceIndexFunc},
...
func podNameNamespaceIndexFunc(obj interface{}) ([]string, error) {
	p, ok := obj.(*object.Pod)
	if !ok {
		return nil, errObj
	}
...
func (dns *dnsControl) PodIndex(ip string) (pods []*object.Pod) {
	os, err := dns.podLister.ByIndex(podNameNamespaceIndex, ip)
	if err != nil {
		return nil
	}
	for _, o := range os {
